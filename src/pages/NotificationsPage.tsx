...
const NotificationsPage: React.FC<NotificationsPageProps> = ({ onNavigate, onViewTicket }) => {
    ...
    const getIconAndColor = (title: string) => {
        ...
        if (t.includes('approved') || t.includes('confirmed'))
            return { icon: <CheckCircle size={22} weight="fill" className="text-green-500 dark:text-green-400" />, bg: 'bg-green-50 dark:bg-green-900/20' };
        if (t.includes('rejected') || t.includes('denied'))
            return { icon: <XCircle size={22} weight="fill" className="text-red-500 dark:text-red-400" />, bg: 'bg-red-50 dark:bg-red-900/20' };
        if (t.includes('cancel'))
            return { icon: <XCircle size={22} weight="fill" className="text-slate-400 dark:text-slate-500" />, bg: 'bg-slate-100 dark:bg-slate-800/40' };
        if (t.includes('confirmed') || t.includes('cancelled'))
            return { icon: <CheckCircle size={22} weight="fill" className="text-green-500 dark:text-green-400" />, bg: 'bg-green-50 dark:bg-green-900/20' };
        return { icon: <Info size={22} weight="fill" className="text-primary" />, bg: 'bg-blue-50 dark:bg-blue-900/20' };
    };
    ...
    return (
        <div className="max-w-2xl mx-auto px-3 sm:px-6 py-4 sm:py-8 pb-24 md:pb-8">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">Notifications</h1>
                ...
                {unreadCount > 0 && (
                    <button
                        ...
                        className="flex items-center gap-2 text-sm font-semibold text-primary hover:text-primary-dark transition-colors bg-primary/10 hover:bg-primary/20 dark:bg-primary/20 dark:hover:bg-primary/30 px-4 py-2 rounded-lg"
                    >
                        <Checks size={18} />
                        Mark all read
                    </button>
            ...
            <div className="flex gap-2 mb-6 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit">
                <button
                    ...
                    className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${filter === 'all' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 shadow-[0_8px_32px_0_rgba(31,38,135,0.05)]-[0_8px_32px_0_rgba(31,38,135,0.05)]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
                >
                    All
                </button>
                <button
                    onClick={() => setFilter('unread')}
                    className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${filter === 'unread' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 shadow-[0_8px_32px_0_rgba(31,38,135,0.05)]-[0_8px_32px_0_rgba(31,38,135,0.05)]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
                >
                    Unread
                    {unreadCount > 0 && (
            ...
            {isLoading && (
                <div className="space-y-3">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 dark:border-slate-800 animate-pulse">
            ...
            {!isLoading && displayed.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    <div className="bg-slate-100 dark:bg-slate-800 p-6 rounded-full mb-4">
                        <BellSlash size={40} className="text-slate-400 dark:text-slate-500" />
                    </div>
                        ...
                        return (
                            <div
                                ...
                                className={`
                                    ...
                                    ${clickable ? 'cursor-pointer hover:shadow-[0_8px_32px_0_rgba(31,38,135,0.05)]-[0_8px_32px_0_rgba(31,38,135,0.05)] active:scale-[0.99]' : 'cursor-default'}
                                    ${!notif.isRead
                                        ? 'bg-blue-50/50 dark:bg-blue-900/10 border-blue-200 dark:border-blue-800/40 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20'
                                        : 'bg-white dark:bg-slate-900/40 border-slate-100 dark:border-slate-800/40 hover:bg-transparent dark:hover:bg-slate-800/60 hover:border-slate-200 dark:hover:border-slate-700/60'
                                    }
                                `}
                            >
                                {/* Icon */}
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between gap-2">
                                        <h4 className={`text-sm leading-snug ${!notif.isRead ? 'font-bold text-slate-800 dark:text-slate-100' : 'font-semibold text-slate-600 dark:text-slate-400'}`}>
                                            {notif.title}
                                        </h4>
                                    ...
                                    {actionLabel && (
                                        <div className="flex items-center gap-1 mt-2">
                                            {isNavigating ? (
                                                <span className="text-[11px] text-primary dark:text-primary-light font-semibold animate-pulse">Opening...</span>
                                            ) : (
                                                <span className="text-[11px] text-primary dark:text-primary-light font-semibold flex items-center gap-1 group-hover:underline">
                                                    {actionLabel}
            ...
            {/* Info bar */}
            <div className="mt-8 bg-transparent dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex items-start gap-3">
                <Bell size={18} className="text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" />
